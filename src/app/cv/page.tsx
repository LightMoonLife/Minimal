  ...
  return (
    <div className="max-w-content mx-auto px-6 sm:px-10">
      ...
      <section className="pt-24 sm:pt-32 lg:pt-40 pb-16 sm:pb-20">
        ...
        <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold text-foreground leading-[.95] tracking-tight mb-6">
          Jack Paul Brookes
        </h1>
        <p className="text-lg font-extralight text-foreground leading-relaxed max-w-lg">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
          {impactMetrics.map((metric) => (
            <div key={metric.label} className="border border-border/10 rounded-card p-5 bg-panel/50 space-y-1.5">
              <p className="font-display text-3xl sm:text-4xl font-extrabold text-foreground tracking-tighter leading-none">
                {metric.value}
              </p>
          ...
          {experience.map((role, i) => (
            <li key={i} className="border border-border/10 rounded-card p-6 sm:p-8">
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1">
                <h2 className="font-display text-base font-extrabold text-foreground">{role.title}</h2>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 mb-4">
                <span className="text-xs font-extralight text-muted-foreground">{role.period}</span>
                <span className="text-xs font-extralight text-muted-foreground">{role.location}</span>
              </div>
              <p className="text-sm font-extralight text-muted-foreground leading-relaxed mb-5 max-w-lg">
                {role.description}
              </p>
              <ul className="space-y-2" aria-label="Role highlights">
                {role.highlights.map((h, j) => (
                  <li key={j} className="flex gap-3">
                    ...
                    <span className="text-sm font-extralight text-foreground leading-relaxed">{h}</span>
                  </li>
                ))}
              </ul>
            </li>
      ...
      <section className="py-16 sm:py-20 border-t border-border/10" aria-labelledby="education-heading">
        ...
        <ol className="space-y-4" aria-label="Education history">
          {education.map((ed, i) => (
            <li key={i} className="border border-border/10 rounded-card p-6 sm:p-8">
              <h2 className="font-display text-base font-extrabold text-foreground mb-1">{ed.degree}</h2>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 mb-3">
                ...
                <span className="text-xs font-extralight text-muted-foreground">{ed.year}</span>
              </div>
              {ed.notes && (
                <p className="text-sm font-extralight text-muted-foreground">{ed.notes}</p>
              )}
            </li>
          ))}
        </ol>
            ...
            <div key={group.label} className="border border-border/10 rounded-card p-6 sm:p-8">
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map(item => (
                  <span key={item} className="text-sm font-extralight text-foreground border border-border/10 px-3 py-1.5 rounded-pill">
                    {item}
                  </span>
                ))}
              </div>
